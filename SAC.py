...
def squash(mu, pi, log_pi, power):
    ...
    """
    mu = torch.tanh(mu) * power[:,None] # broadcast power along the second dimension for multiplication
    if pi is not None:
        pi = torch.tanh(pi) * power[:,None]
    if log_pi is not None:
        log_pi -= torch.log(power[:,None] * (F.relu(1 - pi.pow(2)) + 1e-6)).sum(-1, keepdim=True)
    return mu, pi, log_pi
