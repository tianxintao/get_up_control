        ...
        for _ in range(80):
            ...
            if self.args.to_file:
                ...
                for name in self.geom_names:
                    ...
                    self.starting_geoms[name+"_angleaxis"].append(utils.rotmatrix_to_angleaxis(self.env.physics.named.data.geom_xmat[name].copy().reshape(3, 3)))
...
class HumanoidStandupVelocityEnv(HumanoidStandupEnv):
    ...
    def run_one_episode(self, test_time=False):
        ...
        while True:
            ...
            if len(self.teacher_env.starting_images) > 0:
                self.starting_images = np.concatenate((self.teacher_env.starting_images, self.teacher_env.starting_images, self.teacher_env.starting_images), axis=2)
            if self.args.to_file: 
            ...
            while not done:
                ...
                if self.args.to_file:
                    ...
                    for name in self.geom_names:
                        self.teacher_geoms[name+"_pos"].append(self.teacher_env.physics.named.data.geom_xpos[name].copy())
                        self.teacher_geoms[name+"_angleaxis"].append(utils.rotmatrix_to_angleaxis(self.teacher_env.physics.named.data.geom_xmat[name].copy().reshape(3, 3)))
    ...
    def reset(self, test_time=False, store_buf=False, speed=None):
        ...
        if self.teacher_policy == None:
            ...
            while True:
                ...
                if not test_time and np.random.uniform(0, 1) > 0.2: self._step_num = np.random.randint(0, self.teacher_episode_length)
                self.interpolated_trajectory["qpos"] = np.append(
                    self.interpolated_trajectory["qpos"],
                    [np.append([0, 0, 1.5, 0, 0, 0, 0], self.standing_pose)],
                    axis=0)
                self.interpolated_trajectory["com"] = np.append(self.interpolated_trajectory["com"], [[0, 0, _STAND_HEIGHT]], axis=0)
                self.interpolated_trajectory["com_ori"] = np.append(self.interpolated_trajectory["com_ori"], [[0, 0 ,1.0]], axis=0)
    ...
    def step(self, a, test_time=False):
        ...
        if self.args.to_file:
            ...
            for name in self.geom_names:
                ...
                self.geom_traj[name+"_angleaxis"].append(utils.rotmatrix_to_angleaxis(self.env.physics.named.data.geom_xmat[name].copy().reshape(3, 3)))
    ...
    def _done(self):
        if self._step_num >= self.teacher_episode_length and self.physics.center_of_mass_position()[2] < 0.5:
            ...
            return True
        if self._step_num < self.teacher_episode_length and np.abs(self.env.physics.center_of_mass_position()[2] - self.interpolated_trajectory["com"][self._step_num - 1][2]) > 0.5:
            self.terminal_signal = True
            return True
    ...
    def _reward(self):
        ...
        if self._step_num < self.teacher_episode_length:
            target_leg_speed = np.array([self.interpolated_trajectory["qvel"][index][11], self.interpolated_trajectory["qvel"][index][17]])
    ...
    def render(self, mode=None):
        ...
        with self.render_env.physics.reset_context():
            self.render_env.physics.data.qpos[:] = self.interpolated_trajectory["qpos"][min(self._step_num-1, len(self.interpolated_trajectory["qpos"])-1)]
            self.render_env.physics.data.qvel[:] = self.interpolated_trajectory["qvel"][min(self._step_num-1, len(self.interpolated_trajectory["qvel"])-1)]
