...
STANDING_POSE = [ 0.02572, -0.11665, -0.05646, -0.03601, -0.03219, 0.05025,
...
class HumanoidStandingEnv(HumanoidStandupEnv):
    ...
    def __init__(self, args, seed):
        data = np.load('data/initial_states.npz')
        self.initial_states_length = len(data["qpos"])
        self.initial_qpos = data["qpos"]
        self.initial_qvel = data["qvel"]
    ...
    def reset(self, test_time=False, store_buf=False, speed=None):
        ...
        with self.physics.reset_context():
            self.env.reset()
            index = np.random.randint(0, self.initial_states_length)
            self.physics.named.data.qpos[:] = self.initial_qpos[index]
            self.physics.named.data.qvel[:] = self.initial_qvel[index]
    ...
    def substep(self, action):
        for _ in range(4 * self.env._n_sub_steps):
            pose_diff = ((STANDING_POSE + action) - self.physics.data.qpos[7:])[self.qpos_to_ctrl_index]
            kp = np.ones_like(self.physics.model.actuator_gear[:, 0])
            kd = kp / 3
            final_action = kp * pose_diff - ((kd * self.physics.data.qvel[6:])[self.qpos_to_ctrl_index])
            self.env._task.before_step(final_action, self.env._physics)
    ...
    @property
    def _reward(self):
        rotation_dist = ((STANDING_POSE - self.physics.data.qpos[7:]) ** 2).sum() / 2
        com_dist = ((COM_HEIGHT - self.physics.center_of_mass_position()[2]) ** 2) * 5
        return self._upright * self._small_control * self._dont_move * np.exp(-rotation_dist) * np.exp(-com_dist)
    ...
    #     feet_distance = np.sqrt(((right_feet_pos - left_feet_pos) ** 2).sum())
    #     feet_reward = rewards.tolerance(feet_distance,
    #                                     bounds=(0, 0.), sigmoid='quadratic',
    #                                     margin=0.1, value_at_margin=0.1)
    #     return (2 + feet_reward) / 3


class HumanoidStandupHybridEnv(HumanoidStandupVelocityEnv, HumanoidStandingEnv):
    qpos_to_ctrl_index = np.array([1, 0, 2, 3, 4, 5, 6, 8, 7, 9, 10, 11, 12, 14, 13, 15, 16, 17, 18, 19, 20])

    def __init__(self, args, seed):
        HumanoidStandupVelocityEnv.__init__(self, args, seed)
    
    def reset(self, test_time=False, store_buf=False, speed=None):
        return HumanoidStandupVelocityEnv.reset(self, test_time=test_time, store_buf=store_buf, speed=speed)

    def step(self, a, test_time=False):
        if self.standup_controller:
            return HumanoidStandupVelocityEnv.step(self, a, test_time=test_time)
        else:
            return HumanoidStandingEnv.step(self, a, test_time=test_time)
        
    def substep(self, action):
        if self.standup_controller:
            HumanoidStandupVelocityEnv.substep(self, action)
        else:
            HumanoidStandingEnv.substep(self, action)
    ...
    @property
    def _done(self):
        if self._step_num < (self.teacher_episode_length + 250):
            return False
        return True
...
#                     self.physics.after_reset()
#                 utils.randomize_limited_and_rotational_joints(self.physics, k=0.2)
