...
"""
import torch
from torch.distributions import Categorical, OneHotCategorical, kl_divergence, RelaxedOneHotCategorical
...
class GaussianMixture(Distribution):
    def __init__(self, normal_means, normal_stds, weights):
        ...
        self.categorical = RelaxedOneHotCategorical(torch.tensor([0.1], device=device), probs=self.weights[:, :, 0])

    def log_prob(self, value, squash=False, scalar=1.0):
        # def each_logprob(dist, value, squash, scalar):
        #     scalar = scalar[:, None]
        #     unsquashed = dist.log_prob(value)
        #     if not squash:
        #         return unsquashed
        #     squashed = unsquashed - torch.log(scalar * (F.relu(1 - value.pow(2)) + 1e-6)).sum(-1)
        #     return squashed
        # log_p = [each_logprob(self.normals[i], value, squash, scalar) for i in range(self.num_gaussians)]
        log_p = [self.normals[i].log_prob(value) for i in range(self.num_gaussians)]
        log_p = torch.stack(log_p, -1)
        # p_x = torch.clamp((log_p.exp() * self.weights[:, :, 0]).sum(dim=1), min=1e-8)
        # return torch.log(p_x).unsqueeze(1)
    ...
    def rsample(self):
        z = (
                ...
                MultivariateDiagonalNormal(
                    torch.zeros(self.normal_means.size(), device=device),
                    torch.ones(self.normal_stds.size(), device=device)
                ).sample()
        )
        z.requires_grad_()
        c = self.categorical.sample()[:, :, None]
